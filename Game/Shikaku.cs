...
namespace Karolinaku.Game
{
    public class Shikaku
    {
        ...
        public Shikaku(int gridRowsNum, int gridColsNum)
        {
        ...
        {
            if (rect.Area <= _area/4 && (rect.Area == 1 || rect.Area == 2 || _randomBool(0.38 + 1/(rect.Area)))) 
            {
                return [rect];
            }
        ...
        private bool _randomBool(double threshold)
        {
            var randomGenerator = Random.Shared;

            return randomGenerator.NextDouble() < threshold;
        }
